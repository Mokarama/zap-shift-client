...
const SendParcel = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const res = await axiosSecure.post("/parcels", parcel); // ✅ POST to backend
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-center">
            ...
            {type === "non-document" && (
              <div>
                ...
                <input {...register("weight")} type="number" step="0.1" className="input input-bordered w-full" placeholder="Enter parcel weight" />
              </div>
            )}
          </div>
        </section>
